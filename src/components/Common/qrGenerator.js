 ...
 * Generates standard QR Code Model 2 matrices for client-side Canvas rendering.
 */
...
/**
 * High-level helper: render QR matrix onto an HTML5 Canvas
 */
export function drawQRCodeToCanvas({
  ...
  const qr = new QRCodeModel(0, QRErrorCorrectLevel[errorCorrection] || QRErrorCorrectLevel.M);
  qr.addData(text);
  qr.make();

  const moduleCount = qr.getModuleCount();
  const scale = size / (moduleCount + margin * 2);
